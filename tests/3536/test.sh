...
timeout --preserve-status 10 $ROOT_DIR/jerryscript/build/bin/jerry $IN_FILE 2>&1 | grep -m 1 -q "ECMA_PROPERTY_GET_TYPE (property) == ECMA_PROPERTY_TYPE_NAMEDDATA || ECMA_PROPERTY_GET_TYPE (property) == ECMA_PROPERTY_TYPE_NAMEDACCESSOR || ECMA_PROPERTY_GET_TYPE (property) == ECMA_PROPERTY_TYPE_VIRTUAL"
